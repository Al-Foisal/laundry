...
class DeliverymanResetPasswordController extends Controller
{
    public function resetPassword(Request $request) {
        return view('deliveryman.auth.reset-password', ['request' => $request]);
    }
    ...
    public function storeForgotPassword(Request $request) {
        ...
        if ($deliveryman && $password) {
            $deliveryman->update(['password' => bcrypt($request->password)]);

            $password = DB::table('password_resets')->where('email', $request->email)->delete();

            return redirect()->route('deliveryman.auth.login')->withToastSuccess('New password reset successfully!!');
        } else {
